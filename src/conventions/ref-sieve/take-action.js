...
import { isTrash, cardValue } from '../../basics/hanabi-util.js';
...
export function take_action(game) {
	...
	if (me.thinksLocked(state, state.ourPlayerIndex)) {
		...
		if (best_clue !== undefined) {
			logger.info('give clue line 110')
			return Utils.clueToAction(best_clue, tableID);
		}
		else {
			logger.info('locked discard action line 114')
			return locked_discard_action;
		}
	}
	...
	if (common.thinksLoaded(state, partner) ||
		...
		(this.turn_count !== 1 && !sarcastic_chop)
	) {
		if (common.thinksLoaded(state, partner)) {
		...
		if (state.clue_tokens !== 8 && !state.inEndgame()) {
			...
			if (state.clue_tokens === 0 || (state.clue_tokens === 1 && (type === 'discard' || (type === 'play' && card.rank === 5)))) {
				logger.info('locked discard action line 158')
				return locked_discard_action;
			}
	...
	if (common.thinksLocked(state, partner)) {
		logger.info('thinks partner locked');
			...
			if (identity !== undefined) {
				...
				if (unlocked_order) {
					if (me.thoughts[unlocked_order].matches({ suitIndex: identity.suitIndex, rank: identity.rank + 1 })){
						logger.info('play unlocking card line 185')
		...
		if (discards.length > 0){
			logger.info('discard discardable line 196')
			return { tableID, type: ACTION.DISCARD, target: discards[0].order };
		}

		if (trash_cards.length > 0){
			logger.info('discard trash line 201')
			return { tableID, type: ACTION.DISCARD, target: trash_cards[0].order };
		}
		if (safe_playables.length > 0) {
			...
			logger.info('play playable that leads to closest card line 215')
	...
	// Partner isn't loaded/locked

	if (sarcastic_chop){
		logger.info('sarcastic chop discard line 225')
	...
	if (playable_sarcastic !== undefined && state.clue_tokens !== 8){
		logger.info('playable sarcastic discard line 231')
	...
	if (direct_connections.length > 0){
		logger.info('play into partners hand? line 244')
	...
	let no_clue_value = 0.5+(1.5 - cardValue(state, me, chop, chop.order))/3
	logger.info('no clue value', no_clue_value)
	/** @type {Clue} */
	let lock_clue;
	...
	if (best_clue_value >= no_clue_value && best_clue_value >  0.25) {
		return Utils.clueToAction(best_clue, tableID);
	}
	if (no_clue_value > 0.25) {
		if (playable_cards.length > 0) {
			logger.info('play playable on line 279')
			return { tableID, type: ACTION.PLAY, target: playable_priorities[priority][0].order };
		}
		if (discards.length > 0) {
			logger.info('discard discardable line 283')
			return { tableID, type: ACTION.DISCARD, target: discards[0].order };
		}
		if (trash_cards.length > 0) {
			logger.info('discard trash line 287')
			return { tableID, type: ACTION.DISCARD, target: trash_cards[0].order };
		}
		logger.info('discard chop line 290')
		return { tableID, type: ACTION.DISCARD, target: hand[0].order };
	}	
			
	// Best clue is too low value, lock
	if (best_clue_value <= 0.25 && no_clue_value <= 0.25 && lock_clue !== undefined){
		logger.warn('give lock clue line 296')
		return Utils.clueToAction(lock_clue, tableID);
	}
	logger.warn('give least worst clue line 299')
	return Utils.clueToAction(best_clue, tableID);
}
