...
import { isTrash, refer_right, refer_left } from '../../basics/hanabi-util.js';
...
export function interpret_clue(game, action) {
	...
	// Revealing a playable never is additionally referential, except colour clues where some new cards are touched
	if (!(clue.type === CLUE.COLOUR && touch.some(c => c.newly_clued)) && (new_playable || new_trash)) {
		...
		// Referential play (left)
		if (clue.type === CLUE.COLOUR || trash_push) {
			if (newly_touched.length > 0) {
				const referred = newly_touched.map(index => refer_left(hand, index));
				const target_index = referred.reduce((min, curr) => Math.min(min, curr));
				...
				// Colour fill Bluff
				logger.info('colour fill in, bluff on slot 1', logCard(hand[0]));
				common.thoughts[hand[0].order].finessed = true;
			}
		}
		// Referential discard (right)
			...
			if (newly_touched.length > 0) {
				...
				if (common.thoughts[hand[newly_touched[0]].order].inferred.every(i => state.isPlayable(i))) {
					...
					const referred = newly_touched.map(index => Math.max(0, Utils.nextIndex(hand, (card) => !card.clued, index)));
					const target_index = referred.reduce((min, curr) => Math.min(min, curr));
					// Lock if telling chop to discard
					if (hand[target_index].newly_clued || (target_index === 0 && !common.thinksLoaded(state, target))) {
				...
				// Fill-in (prompt/finesse) - TO DO
				logger.info('rank fill in, prompt/finesse - TO DO');
			}
		}
	}
