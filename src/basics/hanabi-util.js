...
/**
 * Finds the index to the left referred to by the given index.
 * @param  {Hand} hand
 * @param  {number} index
 */
export function refer_left(hand, index) {
	let target_index = (index + hand.length - 1) % hand.length;

	while (hand[target_index].clued && !hand[target_index].newly_clued)
		target_index = (target_index + hand.length - 1) % hand.length;
